...
// Accessing DOM Elements (Querying)
  ...
  function shortenPageTitle() {
      document.querySelector('#main-title').innerText = "What up, I'm DOM.";
  }
  ...
  function RemoveChicagoRace() {
      const races = document.querySelectorAll('#past-races');

      for (let i = 0; i < races[0].children.length; i++) {
          if (races[0].children[i].innerText === "Chicago") {
              races[0].children[i].remove();
          }
      }
  }
...
// Creating New DOM Elements
  ...
  function addPastRace() {
      ...
      newRace.innerText = 'Milwaukee';

      races[0].appendChild(newRace);
  }
  ...
  function createNewBlogPost() {
      ...
      blogDiv.classList.add('blog-post', 'purple');

      const blogHeader = document.createElement('h1');
      blogHeader.innerText = "Milwaukee";

      const blogText = document.createElement('p');
      blogText.innerText = "I drove my car in a wheat field"

      blogDiv.appendChild(blogHeader);
      blogDiv.appendChild(blogText);
      main.appendChild(blogDiv);
  }
